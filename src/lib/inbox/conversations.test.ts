...
describe('matchesAttentionFilter', () => {
  it('matches only conversations with unread messages', () => {
    const unread = { ...makeConversation(null), unread_count: 2 };
    expect(matchesAttentionFilter(unread, 'unread')).toBe(true);
    expect(matchesAttentionFilter(makeConversation(null), 'unread')).toBe(
      false
    );
  });

  it('treats the latest inbound message as awaiting a reply regardless of read state', () => {
    const read = {
      ...makeConversation(null),
      unread_count: 0,
      last_message_sender_type: 'customer' as const,
    };
    const unread = { ...read, unread_count: 3 };
    expect(matchesAttentionFilter(read, 'awaiting_reply')).toBe(true);
    expect(matchesAttentionFilter(unread, 'awaiting_reply')).toBe(true);
  });

  it('does not await a reply after an agent or bot response', () => {
    for (const sender of ['agent', 'bot'] as const) {
      expect(
        matchesAttentionFilter(
          { ...makeConversation(null), last_message_sender_type: sender },
          'awaiting_reply'
        )
      ).toBe(false);
    }
