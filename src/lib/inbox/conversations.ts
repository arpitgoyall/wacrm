...
export type InboxAttentionFilter = 'all' | 'unread' | 'awaiting_reply';

/**
 * Inbox attention filters are deliberately independent of conversation status.
 * A thread awaits a reply whenever its newest message came from the customer,
 * whether or not the agent has already read it.
 */
export function matchesAttentionFilter(
  ...
  if (filter === 'awaiting_reply') {
    return conversation.last_message_sender_type === 'customer';
  }
  return true;
}
