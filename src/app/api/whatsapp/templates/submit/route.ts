...
export async function POST(request: Request) {
  try {
    ...
    if (dryRun) {
      ...
      // Image/video/document headers need a Resumable-Upload handle
      // (Meta rejects a plain URL at creation with "Missing sample
      // parameter"). Derive it from header_media_url before building
      // the payload. Surfaces a 400 with an actionable message (missing
      // META_APP_ID, unreachable URL, wrong type/size).
      try {
        await ensureMediaHeaderHandle(payload, accessToken)
      } catch (e) {
        return NextResponse.json(
          { error: e instanceof Error ? e.message : 'Header file upload failed.' },
          { status: 400 },
        )
      }
