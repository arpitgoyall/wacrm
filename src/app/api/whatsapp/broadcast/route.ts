...
interface NewRecipient {
  phone: string;
  /** Dashboard sends include the existing contact for inbox mirroring. */
  contact_id?: string;
    ...
    for (const recipient of recipients) {
      ...
      if (sentMessageId) {
        try {
          ...
          await persistBroadcastMessage(admin, {
            accountId,
            auditUserId: userId,
            contactId,
          ...
          // Delivery already succeeded at Meta. Keep the recipient sent and
          // surface the persistence failure in server logs for recovery.
          console.error(
            '[broadcast] sent via Meta but failed to mirror into inbox:',
            inboxError
          );
        }
        results.push({
