...
import type { Pipeline, PipelineStage, StageCustomField } from '@/types';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function PipelineSettings({
  ...
  const stageFieldsValid = localStages.every((stage) =>
    (stage.custom_fields ?? []).every(
      (field) =>
        !!field.label.trim() &&
        (field.type !== 'dropdown' ||
        ...
        {showDeleteConfirm ? (
          ...
          <>
            ...
            <DialogFooter className="border-border bg-popover/50">
              ...
              <Button
                onClick={handleSave}
                disabled={saving || !name.trim() || !stageFieldsValid}
...
function SortableStageRow({
  ...
  onRemove: () => void;
  onCustomFieldsChange: (fields: StageCustomField[]) => void;
  colors: string[];
  ...
  return (
    ...
    >
      ...
      <div className="border-border/70 mt-2 space-y-2 border-t pt-2 pl-6">
        ...
        {(stage.custom_fields ?? []).map((field, fieldIndex) => (
          <div
            key={field.id}
            className="bg-background/50 space-y-2 rounded-md p-2"
          >
            <div className="flex items-center gap-2">
              <Input
                value={field.label}
                onChange={(event) => {
                  const fields = [...(stage.custom_fields ?? [])];
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  fields[fieldIndex] = {
                    ...field,
                    type: event.target.value as 'text' | 'dropdown',
                  };
                  onCustomFieldsChange(fields);
                }}
