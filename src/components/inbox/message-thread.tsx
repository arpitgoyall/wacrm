    ...
    (async () => {
      ...
      if (error) {
        ...
        const templateNames = Array.from(
          new Set(
            loadedMessages
              .filter(
                (message) =>
                  message.content_type === 'template' && message.template_name
              )
        ...
        if (templateNames.length === 0) {
          ...
          // Message rows keep the rendered body and delivery data. Hydrate
          // presentation-only template structure from the account catalog so
          // the inbox can show the header, footer, and buttons as well.
          const { data: templateRows, error: templateError } = await supabase
            .from('message_templates')
            .select(
              'name, header_type, header_content, header_media_url, header_handle, footer_text, buttons'
            )
              ...
              loadedMessages.map((message) => {
                ...
                return media
                  ? {
                      ...
                      template_preview: media.template_preview,
                    }
                  : message;
              })
            );
        ...
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          <div className="min-w-0">
            ...
            <a
              ...
              className="text-muted-foreground hover:text-primary inline-flex items-center gap-1 truncate text-xs hover:underline"
            >
